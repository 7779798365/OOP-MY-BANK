...
import { CUSTOMERS_INFO, ACCOUNT_BALANCE } from "./Customer.js";
    ...
    async function OOP_MYBANK(){
        ...
        async function ACCOUNTS(){
           await Anim();
            const { Accounts }: { Accounts:string } = await inquirer.prompt([
                {
       ...
       const account = await ACCOUNTS();
        ...
        const CREATE_ACCOUNT = async () => {
            ...
            const enum SignUp{
                ...
                UserContact_Number = "Create Your Contact Number: [(Contact Number: Must be Pakistani Number Example: +92123456789 OR 03123456789)]: ",
                UserEmail = "Create Your Email: example@Example.com/.pk./uk: ",
                ConfirmEmail = "Confirm Your Email: ",
                Create_User_ID = "Create Your User ID: [(ID Must be in Number Your User ID Must Be in 5 Digits)]: ",
                ConfirmUserID = "Confirm Your User ID: ",
                createuserpin= "Create Your User PIN: [(User PIN Must be in Number Your User PIN Must Be in 6 Digits)]: ",
                ConfirmUserPIN = "Confirm Your User PIN: ",
            };
            const Userinputs = async(UserCreation:SignUp,type:unknown) => {
                while(true){
                    ...
                    if (UserCreation === SignUp.FirstName){
                    ...
                    if (UserCreation === SignUp.LastName){
                    ...
                    if (UserCreation === SignUp.username){
                    ...
                    if (UserCreation === SignUp.UserAge){
                    ...
                    if (UserCreation === SignUp.UserContact_Number){
                    ...
                    if (UserCreation === SignUp.UserEmail){
                    ...
                    if (UserCreation === SignUp.ConfirmEmail){
                        if(CreateAccount === UserEmail){
                            ...
                            UserEmail = await Userinputs(SignUp.UserEmail, 'string');
                            continue;
                        }
                    };
    
                    if(UserCreation === SignUp.Create_User_ID){
                    ...
                    if(UserCreation === SignUp.ConfirmUserID){
                        if(CreateAccount && CreateAccount === Create_User_ID){
                            ...
                            Create_User_ID = await Userinputs(SignUp.Create_User_ID, 'string');
                            continue;
                        }
                    };
                    
                    if(UserCreation === SignUp.createuserpin){
                    ...
                    if(UserCreation === SignUp.ConfirmUserPIN){
                        if (CreateAccount === createuserpin){
                            ...
                            createuserpin = await Userinputs(SignUp.createuserpin, 'string');
                            continue;
                        }
                    };
           ...
           let FirstName = await Userinputs(SignUp.FirstName, 'string');
           let LastName = await Userinputs(SignUp.LastName, 'string');
           let username = await Userinputs(SignUp.username, 'string');
           let UserAge = await Userinputs(SignUp.UserAge, 'number');
           let UserContact_Number = await Userinputs(SignUp.UserContact_Number, 'string');
           let UserEmail = await Userinputs(SignUp.UserEmail, 'string');
           let confirmemail = await Userinputs(SignUp.ConfirmEmail, 'string');
           let Create_User_ID = await Userinputs(SignUp.Create_User_ID, 'string');
           let confirm_user_id = await Userinputs(SignUp.ConfirmUserID, 'string');
           let createuserpin = await Userinputs(SignUp.createuserpin, 'string');
           let confirmuserpin = await Userinputs(SignUp.ConfirmUserPIN, 'string');
        ...
        async function SIGN_IN() {
            ...
            enum Login{
                ...
                PIN = "Enter Your Valid 6 Digits PIN: "
            }
        
            const Sign_In = async (user: string, type: string) => {
                while(true){
                const { SignIn }: {SignIn: string & number} = await inquirer.prompt([
                    {
            ...
            if(User1 && User2){
                if(customer){
                    ...
                    await Anim();
                    while (true){
                        const { Selection }: { Selection:string } = await inquirer.prompt([
                            {
                        ...
                        const { User_Choice }: { User_Choice:string } = await inquirer.prompt([
                            {
        ...
        if (account === "Exit"){
            async function Exit(){
                while(true){
                const {Exit_app}:{Exit_app:string} = await inquirer.prompt([
                    {
...
async function Restart(){
    let restart1 = true;
    while(restart1){
        const { restart }: { restart:string } = await inquirer.prompt([
            {
